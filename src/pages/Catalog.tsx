...
const Catalog = () => {
  ...
  const getUrgencyLabel = (urgency?: string | null) => {
    if (urgency === "urgent") return t("dash.client.urg_urgent");
    if (urgency === "same_day") return t("dash.client.urg_same_day");
    return t("ui.obychnaia");
  };
    ...
    <main className="relative min-h-screen overflow-hidden">
        ...
        {rotatingPopularCategories.length > 0 && (
          <div className="max-w-6xl mx-auto mb-8">
            <div className="flex items-center gap-2 mb-3 text-sm font-medium text-muted-foreground">
              ...
              {t("ui.chasto_zakazyvaiut_seichas")}
            </div>
          ...
          <div className="grid lg:grid-cols-4 gap-6 items-end">
            <div className="lg:col-span-2">
              <label className="flex items-center gap-2 text-sm font-medium mb-3">
                ...
                {t("ui.poisk_po_zakazam")}
              </label>
              <input
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                placeholder={t("ui.naprimer_santehnika_prokladka_izoliaciia")}
      ...
      <section className="container mx-auto pb-12 px-6">
        <div className="flex items-center justify-between gap-4 mb-8 flex-wrap">
          <div>
            <h2 className="text-4xl font-display font-bold mb-2">{t("ui.aktualnye_zakazy")}</h2>
            <p className="text-muted-foreground">
              {selectedCat ? `Открытые заказы в категории «${catById[selectedCat]?.label_ru || catById[selectedCat]?.key || t("ui.vybrannaia_kategoriia")}»` : t("ui.poslednie_otkrytye_zakazy_klientov")}
            </p>
          </div>
                ...
                <div className="px-6 pb-6 -mt-14">
                  ...
                  <div className="text-center mt-4">
                    ...
                    <div className="text-sm text-muted-foreground line-clamp-3 min-h-[3.75rem] mb-4">
                      {pro.bio || t("ui.specialist_gotov_obsudit_detali")}
                    </div>
