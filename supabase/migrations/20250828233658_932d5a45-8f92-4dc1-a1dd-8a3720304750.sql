...
-- 2. WALLETS TABLE - Restrict financial data access (uses pro_id, not user_id)
...
-- 3. TRANSACTIONS TABLE - Restrict financial transaction access (uses pro_id)
ALTER TABLE public.transactions ENABLE ROW LEVEL SECURITY;

-- Users can only see their own transactions (as pro or through jobs)
...
SELECT public.log_security_event('CRITICAL_RLS_POLICIES_APPLIED', '{"action": "restricted_all_sensitive_data", "tables": ["profiles", "wallets", "transactions", "admin_sessions", "ratings"]}'::jsonb);