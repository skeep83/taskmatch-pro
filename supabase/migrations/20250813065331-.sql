...
CREATE POLICY audit_log_select_admins ON public.audit_log
  FOR SELECT USING (
    ...
    has_role(auth.uid(), 'tender'::app_role) OR
    has_role(auth.uid(), 'finance'::app_role) OR
    has_role(auth.uid(), 'risk'::app_role) OR
    has_role(auth.uid(), 'content'::app_role) OR
    has_role(auth.uid(), 'kyc'::app_role) OR
    has_role(auth.uid(), 'dispute_manager'::app_role) OR
    has_role(auth.uid(), 'city_manager'::app_role)
  );
-- Insert allowed to service role or (super)admin
