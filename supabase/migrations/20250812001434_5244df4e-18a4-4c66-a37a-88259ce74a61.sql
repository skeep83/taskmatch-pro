-- CORE SCHEMA FOR ServiceHub MVP+ (jobs, tenders, chat, escrow, wallets, kyc, subscriptions, referrals, business)
-- NOTE: avoid FK to auth.users; store user_id as uuid and enforce via RLS
...
DO $$ BEGIN
  ...
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger WHERE tgname = 'trg_pricing_templates_updated_at'
  ) THEN
  ...
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger WHERE tgname = 'trg_wallets_updated_at'
  ) THEN
  ...
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger WHERE tgname = 'trg_escrows_updated_at'
  ) THEN
  ...
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger WHERE tgname = 'trg_business_accounts_updated_at'
  ) THEN
...
-- POLICIES
-- Categories/pricing: readable by all, admin can manage
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE polname='categories_read_all') THEN
    CREATE POLICY categories_read_all ON public.categories FOR SELECT USING (true);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE polname='categories_admin_ins') THEN
    CREATE POLICY categories_admin_ins ON public.categories FOR INSERT WITH CHECK (public.has_role(auth.uid(),'admin'));
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE polname='categories_admin_upd') THEN
    CREATE POLICY categories_admin_upd ON public.categories FOR UPDATE USING (public.has_role(auth.uid(),'admin'));
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE polname='categories_admin_del') THEN
    CREATE POLICY categories_admin_del ON public.categories FOR DELETE USING (public.has_role(auth.uid(),'admin'));
  END IF;

  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE polname='pricing_read_all') THEN
    CREATE POLICY pricing_read_all ON public.pricing_templates FOR SELECT USING (true);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE polname='pricing_admin_ins') THEN
    CREATE POLICY pricing_admin_ins ON public.pricing_templates FOR INSERT WITH CHECK (public.has_role(auth.uid(),'admin'));
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE polname='pricing_admin_upd') THEN
    CREATE POLICY pricing_admin_upd ON public.pricing_templates FOR UPDATE USING (public.has_role(auth.uid(),'admin'));
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE polname='pricing_admin_del') THEN
...
-- Jobs
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE polname='jobs_select_own_or_assigned_or_admin') THEN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE polname='jobs_insert_client_only') THEN
    CREATE POLICY jobs_insert_client_only ON public.jobs FOR INSERT WITH CHECK (client_id = auth.uid());
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE polname='jobs_update_own_or_assigned_or_admin') THEN
...
-- Job photos: owner or assigned pro
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE polname='job_photos_select_by_job_access') THEN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE polname='job_photos_insert_by_owner') THEN
...
-- Tenders: open are public to authenticated; owner full; admin full
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE polname='tenders_select_open_or_owner_or_admin') THEN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE polname='tenders_insert_owner') THEN
    CREATE POLICY tenders_insert_owner ON public.tenders FOR INSERT WITH CHECK (client_id = auth.uid());
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE polname='tenders_update_owner_or_admin') THEN
...
-- Bids: pro can manage own; tender owner can read; admin full
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE polname='bids_select_pro_or_tender_owner_or_admin') THEN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE polname='bids_insert_pro_only') THEN
    CREATE POLICY bids_insert_pro_only ON public.bids FOR INSERT WITH CHECK (pro_id = auth.uid());
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE polname='bids_update_pro_or_admin') THEN
    CREATE POLICY bids_update_pro_or_admin ON public.bids FOR UPDATE USING (pro_id = auth.uid() OR public.has_role(auth.uid(),'admin'));
  END IF;
END $$;

-- Chats & messages: only participants; admin full
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE polname='chats_select_participants_or_admin') THEN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE polname='chats_insert_participants_only') THEN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE polname='chats_update_participants_or_admin') THEN
    CREATE POLICY chats_update_participants_or_admin ON public.chats FOR UPDATE USING (
      client_id = auth.uid() OR professional_id = auth.uid() OR public.has_role(auth.uid(),'admin')
    );
  END IF;

  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE polname='chat_messages_select_participants_or_admin') THEN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE polname='chat_messages_insert_sender_participant') THEN
...
-- Wallets: pro read own; admin update
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE polname='wallets_select_own_or_admin') THEN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE polname='wallets_insert_admin') THEN
    CREATE POLICY wallets_insert_admin ON public.wallets FOR INSERT WITH CHECK (public.has_role(auth.uid(),'admin'));
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE polname='wallets_update_admin') THEN
    CREATE POLICY wallets_update_admin ON public.wallets FOR UPDATE USING (public.has_role(auth.uid(),'admin'));
  END IF;
END $$;

-- Transactions: pro read own; admin full
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE polname='transactions_select_own_or_admin') THEN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE polname='transactions_insert_admin') THEN
    CREATE POLICY transactions_insert_admin ON public.transactions FOR INSERT WITH CHECK (public.has_role(auth.uid(),'admin'));
  END IF;
END $$;

-- Escrows: parties read; admin update
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE polname='escrows_select_parties_or_admin') THEN
    CREATE POLICY escrows_select_parties_or_admin ON public.escrows FOR SELECT USING (
      client_id = auth.uid() OR pro_id = auth.uid() OR public.has_role(auth.uid(),'admin')
    );
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE polname='escrows_insert_admin') THEN
    CREATE POLICY escrows_insert_admin ON public.escrows FOR INSERT WITH CHECK (public.has_role(auth.uid(),'admin'));
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE polname='escrows_update_admin') THEN
    CREATE POLICY escrows_update_admin ON public.escrows FOR UPDATE USING (public.has_role(auth.uid(),'admin'));
  END IF;
END $$;

-- KYC: user manages own docs; admin reviews
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE polname='kyc_select_own_or_admin') THEN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE polname='kyc_insert_own') THEN
    CREATE POLICY kyc_insert_own ON public.kyc_documents FOR INSERT WITH CHECK (user_id = auth.uid());
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE polname='kyc_update_admin') THEN
    CREATE POLICY kyc_update_admin ON public.kyc_documents FOR UPDATE USING (public.has_role(auth.uid(),'admin'));
  END IF;
END $$;

-- Ratings: only participants
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE polname='ratings_select_involved_or_admin') THEN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE polname='ratings_insert_from_user') THEN
    CREATE POLICY ratings_insert_from_user ON public.ratings FOR INSERT WITH CHECK (from_user_id = auth.uid());
  END IF;
END $$;

-- Subscriptions: user reads own; updates via trusted code
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE polname='subscriptions_select_own_or_admin') THEN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE polname='subscriptions_upsert_any_trusted') THEN
    CREATE POLICY subscriptions_upsert_any_trusted ON public.subscriptions FOR INSERT WITH CHECK (true);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE polname='subscriptions_update_trusted') THEN
...
-- Referrals
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE polname='referrals_select_referrer_or_referred_or_admin') THEN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE polname='referrals_insert_referrer') THEN
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE polname='biz_accounts_select_owner_or_member_or_admin') THEN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE polname='biz_accounts_insert_owner_only') THEN
    CREATE POLICY biz_accounts_insert_owner_only ON public.business_accounts FOR INSERT WITH CHECK (owner_id = auth.uid());
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE polname='biz_accounts_update_owner_or_admin') THEN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE polname='biz_members_select_self_or_admin') THEN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE polname='biz_members_insert_owner_only') THEN
...
-- STORAGE POLICIES
-- KYC: user can read/write own folder; admins full
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE polname='storage_kyc_read_own_or_admin') THEN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE polname='storage_kyc_write_own') THEN
...
-- Evidence: participants of job can read/write their folder
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE polname='storage_evidence_read') THEN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE polname='storage_evidence_write') THEN
...
-- Chat attachments: participants of chat
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE polname='storage_chat_read') THEN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE polname='storage_chat_write') THEN
