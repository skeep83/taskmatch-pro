-- Public catalog enablement + rating aggregates (fixed: no IF NOT EXISTS for policies)

-- 1) Public SELECT for pro profiles and pro categories
CREATE POLICY "pro_profiles_public_select"
ON public.profiles
FOR SELECT
USING (auth.uid() = id);
-- keep existing policies for profiles; do not change
...
-- For pro_categories
CREATE POLICY "pro_categories_public_select"
