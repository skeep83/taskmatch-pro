...
BEGIN
  -- Get current role
  SELECT role INTO current_role FROM public.user_roles WHERE user_id = _user_id;
  
  -- If no role exists, default to client
  IF current_role IS NULL THEN
    current_role := 'client'::app_role;
  END IF;
  
  -- Check upgrade path
  CASE _new_role
    WHEN 'pro' THEN
      can_upgrade := (current_role = 'client');
    WHEN 'business' THEN
      can_upgrade := (current_role IN ('client', 'pro'));
    ELSE
      can_upgrade := false;
  END CASE;
  ...
  IF can_upgrade THEN
    ...
    VALUES (_user_id, _new_role, current_role, now())
    ON CONFLICT (user_id) 
    DO UPDATE SET 
      role = _new_role,
      upgraded_from = current_role,
      upgraded_at = now(),
      updated_at = now();
