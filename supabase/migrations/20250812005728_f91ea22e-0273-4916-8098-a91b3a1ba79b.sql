-- Pro features: profiles, availability, payouts, portfolio, storage
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE polname = 'pro_availability_owner_select') THEN
    CREATE POLICY pro_availability_owner_select ON public.pro_availability FOR SELECT USING (auth.uid() = user_id);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE polname = 'pro_availability_owner_insert') THEN
    CREATE POLICY pro_availability_owner_insert ON public.pro_availability FOR INSERT WITH CHECK (auth.uid() = user_id);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE polname = 'pro_availability_owner_update') THEN
    CREATE POLICY pro_availability_owner_update ON public.pro_availability FOR UPDATE USING (auth.uid() = user_id);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE polname = 'pro_availability_owner_delete') THEN
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE polname = 'payout_requests_owner_insert') THEN
    CREATE POLICY payout_requests_owner_insert ON public.payout_requests FOR INSERT WITH CHECK (pro_id = auth.uid());
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE polname = 'payout_requests_owner_select') THEN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE polname = 'payout_requests_admin_update') THEN
...
DO $$ BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'set_updated_at_pro_profiles') THEN
    CREATE TRIGGER set_updated_at_pro_profiles
...
DO $$ BEGIN
  -- public read
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE polname = 'portfolio_public_read'
  ...
  -- owner write: path starts with user_id
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE polname = 'portfolio_owner_write'
      AND tablename = 'objects' AND schemaname = 'storage'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE polname = 'portfolio_owner_update'
      AND tablename = 'objects' AND schemaname = 'storage'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE polname = 'portfolio_owner_delete'
      AND tablename = 'objects' AND schemaname = 'storage'
  ) THEN
