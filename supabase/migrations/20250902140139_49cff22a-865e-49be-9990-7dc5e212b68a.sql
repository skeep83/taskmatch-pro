...
-- Update user_roles policies to restrict admin role management to super admin only
DROP POLICY IF EXISTS "user_roles_admin_insert" ON public.user_roles;
DROP POLICY IF EXISTS "user_roles_admin_update" ON public.user_roles;
DROP POLICY IF EXISTS "user_roles_admin_delete" ON public.user_roles;

-- Only super admin can insert admin roles
...
WITH CHECK (
  CASE 
    ...
    THEN is_super_admin()
    ELSE user_id = auth.uid() -- Users can still add their own basic roles
  END
);
...
-- Create admin role management function that only super admin can use
